    ...
    if remove is True:
        payload = {
            "people": [
                {
                    "person_id": x,
                    "person_id_type": "Voterfile VAN ID",
                    "action": "removed",
                }
                for x in van_ids
            ]
...
def bq_to_reach(table_name, tag_name):
    ...
    r = requests.get("https://api.reach.vote/api/v1/tags", headers=api_call_headers)
    ...
    if len(voters) > 0:
        batches = [
            voters[i * 1000 : (i + 1) * 1000]
            for i in range((len(voters) + 1000 - 1) // 1000)
        ]
        for batch in batches:
            put = update_people_to_tag(batch, id, api_call_headers)
